...
const check = async (email, password, expected) => {
  ...
  const role = (await page.locator('header span', { hasText: /^(Admin|Manager|Operator)$/ })
  ...
  await page.screenshot({ path: `${OUT}/rol-${expected.role.toLowerCase()}-productos.png` });
  ...
  await page.screenshot({ path: `${OUT}/rol-${expected.role.toLowerCase()}-movimientos.png` });
  ...
  const reachedUsers = await page.locator('text=Sección exclusiva de Admin').count();
  const expectBlocked = expected.role !== 'Admin';
  ...
  if (expectBlocked) await page.screenshot({ path: `${OUT}/rol-${expected.role.toLowerCase()}-sin-acceso.png` });
...
await check('admin@stockhex.local', 'Admin123456', {
  role: 'Admin', canCreate: 1, canReverse: true,
  nav: ['Dashboard', 'Productos', 'Movimientos', 'Reportes', 'Categorías', 'Proveedores', 'Clientes', 'Usuarios'],
});

await check('carla@stockhex.cl', 'Manager1234', {
  role: 'Manager', canCreate: 1, canReverse: true,
  nav: ['Dashboard', 'Productos', 'Movimientos', 'Reportes', 'Categorías', 'Proveedores', 'Clientes'],
});

await check('juan@stockhex.cl', 'Operario123', {
  role: 'Operator', canCreate: 0, canReverse: false,
